...
export class UsersListComponent implements OnInit, OnDestroy {
	...
	displayedColumns = ['select', 'id', 'username', 'email', 'fullname', 'occupation', '_roles', 'actions'];
	...
	ngOnInit() {
		...
		this.authServiceApp.getDataSelect(Occupation, 'api/users/occupation').subscribe(res => {
				if(res) {
	...
	getOccupationStr(user: User): string {
		var titles: string = "";
		const _occu = find(this.occupations, (occu: Occupation) => occu.occupation === user.occupation);
		if(_occu){
			titles = _occu.descr;
		}
