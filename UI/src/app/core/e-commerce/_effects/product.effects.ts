...
export class ProductEffects {
	...
	loadProductsPage$ = this.actions$.pipe(
		...
		mergeMap(({ payload }) => {
			...
			const requestToServer = this.productsService.findProducts(
				payload.page
			);
	...
	@Effect()
	updateProductsStatus$ = this.actions$.pipe(
		ofType<ProductsStatusUpdated>(ProductActionTypes.ProductsStatusUpdated),
		mergeMap(({ payload }) => {
			this.store.dispatch(this.showLoadingDistpatcher);
			return this.productsService.updateStatusForProduct(
				payload.products,
				payload.active
			);
		}),
	...
	createProduct$ = this.actions$.pipe(
		ofType<ProductOnServerCreated>(
			ProductActionTypes.ProductOnServerCreated
		),
		mergeMap(({ payload }) => {
			this.store.dispatch(this.showLoadingDistpatcher);
